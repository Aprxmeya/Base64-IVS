  ...
  return (
    <FormField
      ...
      render={({ field }) => (
        <FormItem>
          <FormControl>
            <div className="rounded-lg border bg-card p-6 flex flex-col md:flex-row gap-6 items-center">
              <div className="w-full md:w-1/3 flex flex-col items-center gap-4">
                 <div className="w-48 h-64 rounded-lg bg-muted flex items-center justify-center overflow-hidden border-2 border-dashed">
                    {photoValue ? (
                        <Image src={photoValue} alt="User photo" width={192} height={256} className="object-cover w-full h-full" />
              ...
              <div className="w-full md:w-2/3 space-y-2 text-center md:text-left">
                 <h3 className="font-semibold text-lg">Profile Photograph</h3>
                 <p className="text-sm text-muted-foreground">
                   Please upload a clear, forward-facing photo. This will be used for your profile.
                 </p>
                ...
                <div className={cn("transition-all duration-300", mode === 'upload' ? 'opacity-100' : 'opacity-0 h-0 overflow-hidden')}>
                     <input type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept="image/png, image/jpeg, image/webp" />
                     <p className="text-xs text-muted-foreground mt-2 text-center md:text-left">Max 4MB. PNG, JPG, WEBP.</p>
                </div>
